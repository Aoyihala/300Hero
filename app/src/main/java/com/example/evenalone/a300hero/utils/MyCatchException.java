...
import com.example.evenalone.a300hero.bean.ErroBean;
...
public class MyCatchException implements Thread.UncaughtExceptionHandler {
        ...
        new Thread() {
            public void run() {
               ...
               //将异常记录到本地的数据库或者文件中.或者直接提交到后台服务器
                ...
                erroBean.setTime(time);
                erroBean.setMessage("异常:"+msg);
                Looper.loop();
            }
    ...
    public void uncaughtException(Thread thread, Throwable ex) {
        if (!handlerException(ex) && mDefaultException != null) {
            ...
            String time = dateFormat.format(System.currentTimeMillis());
            erroBean.setTime(time);
            erroBean.setTitle("未及时处理异常");
            erroBean.setMessage(ex.getMessage());
            ...
            try {  //Sleep 来让线程停止一会是为了显示Toast信息给用户，然后Kill程序
                ...
                String time = dateFormat.format(System.currentTimeMillis());
                erroBean.setTime(time);
                erroBean.setMessage(e.getMessage());
